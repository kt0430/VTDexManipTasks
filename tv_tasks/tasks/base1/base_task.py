...
class BaseTask():
    ...
    def __init__(self, cfg, enable_camera_sensors=False, bi_hands = False, is_meta=False, task_num=0):
        ...
        self.img_dim = cfg["env"]["img_dim"]
        self.num_obs_states = 0
        for v in cfg["env"]["obs_dim"].values():
            self.num_obs_states += v
        self.num_actions = cfg["env"]["action_dim"]
        ...
        if cfg["env"]["obs_type"] in ["VisOnly","VisTac"]:
            ...
            #         (self.num_envs, 224, 224, C), device=self.device, dtype=torch.float)
        # # render
        # self.render_buf = torch.zeros(
        #     (self.num_envs, H, W, C), device=self.device, dtype=torch.float)
        # self.render_buf1 = torch.zeros(
        #     (self.num_envs, H, W, C), device=self.device, dtype=torch.float)
        # self.render_buf2 = torch.zeros(
        #     (self.num_envs, H, W, C), device=self.device, dtype=torch.float)
