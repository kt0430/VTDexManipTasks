...
class T_ReTac(nn.Module):
    def __init__(
        ...
        self.encoder_pe_tac = self.TacConNet.Encoder_pe(self.encoder_embed_dim)
        ...
        self.decoder_pe_tac = self.TacConNet.Decoder_pe(self.decoder_embed_dim)
        # 这边不需要cls token了
    ...
    def forward_tac_decoder(self, visible_tactile: torch.Tensor, restore_idxs) -> torch.Tensor:
        ...
        mask_tokens = self.tac_mask_token.repeat(  # (bts, 147, 192)
            ...
            restore_idxs.shape[1] - visible_tactile.shape[1],
            1,
        )
        ...
        decoder_tac = unshuffled_patches + self.decoder_pe_tac
        # print(f"decoder_tac.shape: {decoder_tac.shape}")

        # concate decoder patches
        decoder_patches = decoder_tac
